<template>
  <div class="home">
    <div class="header">
      <div
        class="address_map"
        @click="$router.push({name:'addre',params:{city:location.addressComponent.city}})"
      >
    ...
    <div class="container">
      <mt-swipe :auto="1000" class="swiper">
...
export default {
  ...
  methods:{
    getData(){
      this.$axios.get('/api/profile/shopping').then(response=>{
        this.swipeImgs = response.data.swipeImgs
        this.entries = response.data.entries
      }).catch(()=>{
        
      })
      this.$axios("/api/profile/filter").then(response=>{
