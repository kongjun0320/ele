<template>
  <div class="city">
    <div class="search_wrap">
      ...
      <button @click="$router.push({name:'addre',params:{city:city}})">取消</button>
    </div>
    <div style="height:100%" v-if="searchList.length ==0">
      <div class="location">
        <Location @MyClick="selectCity({name:city})" :address="city" />
      </div>
...
export default {
  ...
  methods: {
    ...
    selectCity(city) {
      // console.log(city);
      this.$router.push({ name: "addre", params: { city: city.name } });
    },
    searchCity() {
      if (!this.city_val) {
        // 如果搜索框为空, 数组置空
